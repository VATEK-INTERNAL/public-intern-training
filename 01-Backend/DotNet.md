...
| | |
|---|---|
| **Language** | .NET / C# — the whole month: labs, both microservices and the API Gateway |
...
| Layer | Technology |
|---|---|
| **Core API** | ASP.NET Core 8 Web API · EF Core 8 (Npgsql) · xUnit · Moq · FluentValidation · Serilog · Swashbuckle |
| **Ops Service** | The same stack — a separate project, its own database tables, its own container |
| **API Gateway** | YARP (`Yarp.ReverseProxy`) — or Traefik / Nginx |
...
- .NET 8 SDK
- Visual Studio 2022, JetBrains Rider, or VS Code + C# Dev Kit
...
| Day | Documentation |
|---|---|
| **D1** | [C# guide](https://learn.microsoft.com/en-us/dotnet/csharp/) · [A tour of C#](https://learn.microsoft.com/en-us/dotnet/csharp/tour-of-csharp/) · [.NET CLI](https://learn.microsoft.com/en-us/dotnet/core/tools/) |
| **D2** | [OOP in C#](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/object-oriented/) · [Interfaces](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/types/interfaces) · [Generics](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/types/generics) · [Dependency injection](https://learn.microsoft.com/en-us/dotnet/core/extensions/dependency-injection/overview) |
| **D3** | [LINQ](https://learn.microsoft.com/en-us/dotnet/csharp/linq/) · [Collections](https://learn.microsoft.com/en-us/dotnet/standard/collections/) · [Async programming](https://learn.microsoft.com/en-us/dotnet/csharp/asynchronous-programming/) · [Exceptions](https://learn.microsoft.com/en-us/dotnet/standard/exceptions/) |
| **D4** | [ASP.NET Core](https://learn.microsoft.com/en-us/aspnet/core/) · [Web API](https://learn.microsoft.com/en-us/aspnet/core/web-api/) · [Middleware](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/middleware/) · [Routing](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/routing) · [Configuration](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/configuration/) |
...
### 🔧 Your stack this week — .NET

| Day | Tools & commands |
|---|---|
| D6 | `dotnet new webapi` · solution with `src/` + `tests/` · Serilog JSON console sink · `IOptions<T>` bound from environment · multi-stage Dockerfile (`sdk` → `aspnet`) · GitHub Actions: `dotnet build` + `dotnet format --verify-no-changes` |
| D7 | EF Core code-first + Npgsql · `dotnet ef migrations add Init` / `dotnet ef database update` · seeding via a hosted seeder or `HasData` |
| D8 | Controllers (or Minimal APIs) · FluentValidation · `AddProblemDetails()` + `UseExceptionHandler()` · Swashbuckle · `Skip`/`Take` pagination |
| D9 | `AddAuthentication().AddJwtBearer()` · `PasswordHasher<T>` or BCrypt.Net · `[Authorize(Roles = "ADMIN")]` · xUnit + Moq · Coverlet for coverage |
| D10 | `BeginTransactionAsync()` · rowversion / `[ConcurrencyCheck]` · `DbUpdateConcurrencyException` · an `idempotency_keys` table |
...
| Day | Tools & commands |
|---|---|
| D11 | `IHttpClientFactory` + Polly (timeout, retry with jitter) · HMAC-SHA256 webhook signature check · `IFormFile` with size/MIME validation |
| D12 | `IDistributedCache` + StackExchange.Redis · `AsNoTracking()` / `Include()` · EF Core SQL logging · `WebApplicationFactory<T>` + Testcontainers for .NET |
| D13 | A second ASP.NET Core project, `Ops.Api`, with its own `OpsDbContext` (tables `reports`, `notifications` only) and its own EF Core migrations · Serilog · Dockerfile |
| D14 | **Core:** MassTransit or RabbitMQ.Client publishing `order.paid` after the transaction commits<br>**Ops:** a MassTransit consumer, or a `BackgroundService` with manual ack, a retry policy and a dead-letter queue |
| D15 | Quartz.NET, or a `BackgroundService` + `PeriodicTimer`, for the nightly order report (idempotent upsert) · mock `IEmailSender` / `ISmsSender` · one compose file for both services |
...
| Day | Tools & commands |
|---|---|
| D16 | **Native:** YARP — routes and clusters in `appsettings.json`; Swagger UI via Swashbuckle with one `SwaggerEndpoint` per service<br>**Or:** Traefik or Nginx as the gateway + the `swaggerapi/swagger-ui` container with `URLS` listing both specs |
...
| D18 | `dotnet list package --vulnerable --include-transitive` · `dotnet user-secrets` for local secrets · verify JWT `ValidateLifetime` |
| D19 | ASP.NET Core Health Checks (`/health/live`, `/health/ready` with DB + RabbitMQ checks) · k6 · `dotnet-counters` |
...
[C# guide](https://learn.microsoft.com/en-us/dotnet/csharp/) · [A tour of C#](https://learn.microsoft.com/en-us/dotnet/csharp/tour-of-csharp/) · [OOP](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/object-oriented/) · [Interfaces](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/types/interfaces) · [Generics](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/types/generics) · [LINQ](https://learn.microsoft.com/en-us/dotnet/csharp/linq/) · [Async](https://learn.microsoft.com/en-us/dotnet/csharp/asynchronous-programming/) · [ASP.NET Core](https://learn.microsoft.com/en-us/aspnet/core/) · [Web API](https://learn.microsoft.com/en-us/aspnet/core/web-api/) · [Middleware](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/middleware/) · [DI](https://learn.microsoft.com/en-us/dotnet/core/extensions/dependency-injection/overview) · [EF Core](https://learn.microsoft.com/en-us/ef/core/) · [Microservices e-book](https://learn.microsoft.com/en-us/dotnet/architecture/microservices/) · [xUnit](https://xunit.net/) · [FluentValidation](https://docs.fluentvalidation.net/en/latest/) · [Polly](https://www.pollydocs.org/) · [Serilog](https://serilog.net/)

### .NET / C# — the libraries used in this track

[EF Core migrations](https://learn.microsoft.com/en-us/ef/core/managing-schemas/migrations/) · [JWT bearer authentication](https://learn.microsoft.com/en-us/aspnet/core/security/authentication/configure-jwt-bearer-authentication) · [IHttpClientFactory](https://learn.microsoft.com/en-us/dotnet/core/extensions/httpclient-factory) · [Distributed caching](https://learn.microsoft.com/en-us/aspnet/core/performance/caching/distributed) · [Integration tests](https://learn.microsoft.com/en-us/aspnet/core/test/integration-tests) · [Rate limiting middleware](https://learn.microsoft.com/en-us/aspnet/core/performance/rate-limit) · [Health checks](https://learn.microsoft.com/en-us/aspnet/core/host-and-deploy/health-checks) · [Testcontainers for .NET](https://dotnet.testcontainers.org/) · [RabbitMQ .NET client](https://www.rabbitmq.com/client-libraries/dotnet) · [MassTransit](https://masstransit.massient.com/concepts) · [Swashbuckle](https://github.com/domaindrivendev/Swashbuckle.AspNetCore) · [YARP](https://microsoft.github.io/reverse-proxy/) · [Hosted services / BackgroundService](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/host/hosted-services) · [Quartz.NET](https://www.quartz-scheduler.net/)

### API Gateway & Swagger aggregation

[YARP reverse proxy](https://microsoft.github.io/reverse-proxy/) · [Swashbuckle](https://github.com/domaindrivendev/Swashbuckle.AspNetCore) · [API gateway pattern](https://microservices.io/patterns/apigateway.html) · [Traefik](https://doc.traefik.io/traefik/) · [Nginx](https://nginx.org/en/docs/) · [Kong](https://developer.konghq.com/) · [Swagger UI configuration (`urls` for multiple specs)](https://swagger.io/docs/open-source-tools/swagger-ui/usage/configuration/)

---

*VATEK Internship Program 2026 · Backend Developer Intern · Track — .NET / C#*
