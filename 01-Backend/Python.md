...
| | |
|---|---|
| **Language** | Python / FastAPI — the whole month: labs, both microservices and the API Gateway |
...
| Layer | Technology |
|---|---|
| **Core API** | FastAPI · Pydantic v2 · SQLAlchemy 2.0 · Alembic · pytest · httpx · structlog |
| **Ops Service** | The same stack — a separate project, its own database tables, its own container |
| **API Gateway** | FastAPI gateway app + httpx — or Traefik / Nginx |
...
| Day | Documentation |
|---|---|
| **D1** | [Python tutorial](https://docs.python.org/3/tutorial/) · [typing](https://docs.python.org/3/library/typing.html) · [uv](https://docs.astral.sh/uv/) · [Ruff](https://docs.astral.sh/ruff/) |
| **D2** | [Classes](https://docs.python.org/3/tutorial/classes.html) · [abc](https://docs.python.org/3/library/abc.html) · [Protocols (PEP 544)](https://peps.python.org/pep-0544/) · [Pydantic](https://pydantic.dev/docs/validation/latest/get-started/) |
| **D3** | [asyncio](https://docs.python.org/3/library/asyncio.html) · [collections](https://docs.python.org/3/library/collections.html) · [itertools](https://docs.python.org/3/library/itertools.html) · [Exceptions](https://docs.python.org/3/tutorial/errors.html) |
| **D4** | [FastAPI](https://fastapi.tiangolo.com/) · [Dependencies](https://fastapi.tiangolo.com/tutorial/dependencies/) · [Middleware](https://fastapi.tiangolo.com/tutorial/middleware/) · [Settings](https://fastapi.tiangolo.com/advanced/settings/) |
| **D5** | [SQLAlchemy ORM](https://docs.sqlalchemy.org/en/20/orm/) · [Alembic](https://alembic.sqlalchemy.org/en/latest/) · [pytest](https://docs.pytest.org/en/stable/) · [httpx](https://www.python-httpx.org/) · [FastAPI testing](https://fastapi.tiangolo.com/tutorial/testing/) |
...
### 🔧 Your stack this week — Python

| Day | Tools & commands |
|---|---|
| D6 | `uv init` · FastAPI app factory · structlog JSON logging · pydantic-settings · multi-stage Dockerfile (`python:3.12-slim`) · GitHub Actions: `uv run ruff check` + `uv run pytest` |
| D7 | SQLAlchemy 2.0 declarative models · `alembic revision --autogenerate` / `alembic upgrade head` · a seed script |
| D8 | `APIRouter` + Pydantic request/response models · exception handlers returning problem+json · built-in OpenAPI at `/docs` · limit/offset pagination |
| D9 | `OAuth2PasswordBearer` + PyJWT · bcrypt (via pwdlib or passlib) · a role dependency, e.g. `Depends(require_role('ADMIN'))` · pytest + pytest-cov |
| D10 | `async with session.begin()` · SQLAlchemy `version_id_col` optimistic locking · `StaleDataError` · an `idempotency_keys` table |
...
| Day | Tools & commands |
|---|---|
| D11 | `httpx.AsyncClient` with timeouts + tenacity retries · HMAC check with `hmac.compare_digest` · `UploadFile` with size/MIME validation |
| D12 | redis-py (asyncio) cache-aside · `selectinload` / `joinedload` against N+1 · SQL echo logging · testcontainers-python + `httpx.AsyncClient` tests |
| D13 | A second FastAPI application, `ops_service`, with its own SQLAlchemy models and Alembic migrations (`reports`, `notifications` only) · structlog · Dockerfile |
| D14 | **Core:** aio-pika publishing `order.paid` after commit<br>**Ops:** an aio-pika consumer with manual ack, prefetch, retry via delayed requeue and a dead-letter exchange |
| D15 | APScheduler cron job for the nightly order report (idempotent upsert) · mock email/SMS sender · one compose file for both services |
...
| Day | Tools & commands |
|---|---|
| D16 | **Native:** a FastAPI gateway app forwarding with `httpx`, serving Swagger UI with a `urls` list for both specs<br>**Or:** Traefik or Nginx as the gateway + the `swaggerapi/swagger-ui` container with `URLS` listing both specs |
...
| D18 | pip-audit · slowapi if not rate-limiting at the gateway · verify JWT `exp` handling |
| D19 | `/health/live` + `/health/ready` routes checking DB and broker · k6 · structlog correlation IDs |
...
[Python tutorial](https://docs.python.org/3/tutorial/) · [typing](https://docs.python.org/3/library/typing.html) · [asyncio](https://docs.python.org/3/library/asyncio.html) · [FastAPI](https://fastapi.tiangolo.com/) · [Pydantic](https://pydantic.dev/docs/validation/latest/get-started/) · [SQLAlchemy](https://docs.sqlalchemy.org/en/20/) · [Alembic](https://alembic.sqlalchemy.org/en/latest/) · [pytest](https://docs.pytest.org/en/stable/) · [httpx](https://www.python-httpx.org/) · [uv](https://docs.astral.sh/uv/) · [Ruff](https://docs.astral.sh/ruff/) · [Celery](https://docs.celeryq.dev/en/stable/)

### Python / FastAPI — the libraries used in this track

[FastAPI security](https://fastapi.tiangolo.com/tutorial/security/) · [Request files](https://fastapi.tiangolo.com/tutorial/request-files/) · [pydantic-settings](https://pydantic.dev/docs/validation/latest/concepts/pydantic_settings/) · [SQLAlchemy ORM](https://docs.sqlalchemy.org/en/20/orm/) · [PyJWT](https://pyjwt.readthedocs.io/en/stable/) · [tenacity](https://tenacity.readthedocs.io/en/latest/) · [redis-py](https://redis.readthedocs.io/en/stable/) · [aio-pika](https://docs.aio-pika.com/) · [testcontainers-python](https://testcontainers-python.readthedocs.io/en/latest/) · [structlog](https://www.structlog.org/en/stable/) · [pip-audit](https://github.com/pypa/pip-audit) · [Traefik](https://doc.traefik.io/traefik/) · [APScheduler](https://apscheduler.readthedocs.io/en/stable/) · [RabbitMQ dead-letter exchanges](https://www.rabbitmq.com/docs/dlx)

### API Gateway & Swagger aggregation

[httpx](https://www.python-httpx.org/) · [FastAPI behind a proxy](https://fastapi.tiangolo.com/advanced/behind-a-proxy/) · [API gateway pattern](https://microservices.io/patterns/apigateway.html) · [Traefik](https://doc.traefik.io/traefik/) · [Nginx](https://nginx.org/en/docs/) · [Kong](https://developer.konghq.com/) · [Swagger UI configuration (`urls` for multiple specs)](https://swagger.io/docs/open-source-tools/swagger-ui/usage/configuration/)

---

*VATEK Internship Program 2026 · Backend Developer Intern · Track — Python / FastAPI*
